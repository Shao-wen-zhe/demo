...
class gatemamba_ablations_ssm(pyg_nn.conv.MessagePassing):
    def __init__(
        self, 
        d_model,
        conv_layes, 
    ...
    def forward(self, x, edge_index, A_norm, edge_attr):
        ...
        # delta = self.dt_proj.weight @ x_dbl[:, :self.dt_rank].t()
    ...
    def aggregate(self, Abar_ij, edge_index, Bx_j, Ax_i, Bx):
        ...
        if self.pool in ['sum', 'add']:
            ...
            sum_sigma_x = sum_sigma_x * self.A_norm.unsqueeze(1).unsqueeze(2)
