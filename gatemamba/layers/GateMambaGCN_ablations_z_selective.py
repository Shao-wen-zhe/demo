...
class GateMambaGCN_ablations_z_selective(pyg_nn.conv.MessagePassing):
    def __init__(
        self, 
        d_model,
        conv_layes, 
        ...
        self.in_proj = nn.Linear(self.d_model, self.d_inner, bias=bias, **factory_kwargs)    # # 双路投影：in_proj 将输入映射到 2*d_inner 维度（含门控信号）对应conv之前和 silu之间
        ...
        self.z = nn.Parameter(torch.randn(self.num_nodes, self.d_inner, **factory_kwargs))
