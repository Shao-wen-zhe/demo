...
class GateMambaGCN_layer_sigmoid(pyg_nn.conv.MessagePassing):
    def __init__(
        self, 
        d_model,
        conv_layes, 
    ...
    def forward(self, x, edge_index, A_norm, edge_attr):
        ...
        C = torch.sigmoid(x_dbl[:, -self.d_state:])
        
        delta = self._pre_delta(delta)
        # delta_u, delta_v = self._pre_delta(delta)
        # 消融实验
        # delta = F.silu(delta).t()
