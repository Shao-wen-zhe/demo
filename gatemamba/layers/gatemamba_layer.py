...
class GateMambaGCN_layer(pyg_nn.conv.MessagePassing):
    def __init__(
        self, 
        d_model,
        conv_layes, 
    ...
    def forward(self, x, edge_index, A_norm, edge_attr):
        ...
        self.A_norm = A_norm
        xz = rearrange(
            self.in_proj.weight @ rearrange(x, "l d -> d l"),    # l: num_nodes, d: d_model
            "d l -> l d",
            l=N,
        )
        ...
        xx = scatter_add(out, row, dim=0, dim_size=N)
        # xx, edge_attr = self.gate_gcn(x, edge_index, edge_attr=None)  # Gated GCN
        h  = F.silu(xx)
        ...
        A_u = torch.einsum('ld,dn->ldn', delta_u, A)    # l: num_nodes, d: d_inner, n: d_state
        A_v = torch.einsum('ld,dn->ldn', delta_v, A)
        
        Bbar_hi = torch.einsum('ln,ld->ldn', B, h)
        # Bbar_hi = torch.einsum('ln,ld->ldn', B, h * delta_u)
        
        # 计算 h_new
        new_h = self.propagate(edge_index, Bx=Bbar_hi, Dx=A_v, Ex=A_u, Ax=Bbar_hi, N=N, flow='source_to_target')
            
        Ch = torch.einsum('ln,ldn->ld', C, new_h)   # naive 
