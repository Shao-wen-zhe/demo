...
class GateMambaGCN_(pyg_nn.conv.MessagePassing):
    def __init__(
        self, 
        d_model,
        top_k,
        ...
        self.x_proj = nn.Linear(self.d_inner, self.dt_rank + self.d_state + self.top_k, bias=False, **factory_kwargs)     # selective projection used to make dt, B and C input dependant
    ...
    def forward(self, x, edge_index, A_norm, edge_attr):
        ...
        selected_A_u = torch.gather(torch.einsum('ld,dn->ldn', delta_u, A), dim=-1, index=topk_indices)
        selected_A_v = torch.gather(torch.einsum('ld,dn->ldn', delta_v, A), dim=-1, index=topk_indices)
        for i in range(self.layers):

            hi = self.propagate(edge_index, Bx=topk_Bbar_hi, Ax=selected_A_v, Cx=selected_A_u, flow='source_to_target')
            selected_Bbar_hi = torch.gather(torch.einsum('ld,ldn->ldn', delta_v, hi), dim=-1, index=topk_indices)
        Ch = torch.einsum('ln,ldn->ld', C, hi)
    ...
    def aggregate(self, Abar_ij, edge_index, Bx_j, Ax_i, Bx):
        ...
        dim_size = Bx.shape[0]
        sum_sigma_x = Abar_ij * Bx_j    # 消融 去除自项
