...
class GateMambaGCN_top_k_layer(pyg_nn.conv.MessagePassing):
    def __init__(
        self, 
        d_model,
        top_k,
        ...
        self.x_proj = nn.Linear(self.d_inner, self.dt_rank + self.d_state + self.d_state, bias=False, **factory_kwargs)
    ...
    def forward(self, x, edge_index, A_norm, edge_attr):
        ...
        B_top, topk_indices = torch.topk(F.softmax(B, dim=-1), k=self.top_k, dim=-1)
        C = x_dbl[:, -self.d_state:] 
        selected_C = torch.gather(C, dim=-1, index=topk_indices)
        
        delta = self._pre_delta(delta)
                
        delta_u, delta_v = delta.chunk(2, dim=-1)
        # delta_u_top = torch.gather(delta_u, dim=-1, index=topk_indices)
        # delta_v_top = torch.gather(delta_v, dim=-1, index=topk_indices)
        delta_uv = None
        if delta_uv is not None:
            Bbar_hi = torch.einsum('ln,ld->ldn', B, (delta_v * h))
            Bbar_hj = torch.einsum('ln,ld->ldn', B, (delta_u * h))
        else:
            Bbar_hi = torch.einsum('ln,ld->ldn', B_top,  h)
            # topk_Bbar_hi, topk_indices = torch.topk(F.softmax(Bbar_hi, dim=-1), k=self.top_k, dim=-1)

        # A_u = torch.einsum('ld,dn->ldn', delta_u_top, A)
        # A_v = torch.einsum('ld,dn->ldn', delta_v_top, A)            
        selected_A_u = torch.gather(torch.einsum('ld,dn->ldn', delta_u, A), dim=-1, index=topk_indices.unsqueeze(1))
        selected_A_v = torch.gather(torch.einsum('ld,dn->ldn', delta_v, A), dim=-1, index=topk_indices.unsqueeze(1))
        new_h = self.propagate(edge_index, Bx=Bbar_hi, Ax=selected_A_v, Cx=selected_A_u, flow='source_to_target')
        # new_h = self.propagate(edge_index, Bx=Bbar_hi, Dx=A_v, Ex=A_u, Ax=Bbar_hi, flow='source_to_target')

        Ch = torch.einsum('ln,ldn->ld', selected_C, new_h)
    ...
    def message(self, Ax_i, Cx_j):
        # self.delta_ij = Dx_i + Ex_j
        Abar_ij = torch.exp(Ax_i + Cx_j)
    ...
    def aggregate(self, Abar_ij, edge_index, Bx_j, Bx):
        ...
        # sum_sigma_x = Abar_ij * Bx_j + Ax_i    # 消融 去除输入门
        # sum_sigma_x =  Bx_j + Bbar_ij * Ax_i     # 消融 去除遗忘门
