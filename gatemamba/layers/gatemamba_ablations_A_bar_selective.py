...
class gatemamba_ablations_A_bar_selective(pyg_nn.conv.MessagePassing):
    def __init__(
        self, 
        d_model,
        conv_layes, 
        ...
        self.dt_proj = nn.Linear(self.dt_rank, self.d_inner, bias=True, **factory_kwargs)        # # time step projection (discretization)
        self.out_proj = nn.Linear(self.d_inner, self.d_model, bias=bias, **factory_kwargs)
        self.norm = nn.LayerNorm(self.d_model)
        # self.gat = pygnn.GATConv(in_channels=self.d_model,out_channels=self.d_model // num_heads, heads=num_heads)
        ...
        # A = repeat(
        #     torch.ones(self.d_state, dtype=torch.float32, device=device),
        #     "n ->  d n",
        #     d=self.d_inner,
        # ).contiguous()
    ...
    def forward(self, x, edge_index, A_norm, edge_attr):
        ...
        # A_delta = delta.t()
        A_delta = torch.einsum('ld,dn->ldn', delta.t(), A)
        ...
        h_src = A_delta[row]  # [E, heads, out_channels]
        h_dst = A_delta[col]  # [E, heads, out_channels]
        # 计算 h_new
        new_h = self.propagate(edge_index, Bx=Bbar_hi, h_src=h_src, h_dst=h_dst, Ax=Bbar_hi, N=N,flow='source_to_target')
