...
def prepare_splits(dataset, args):
    ...
    if split_mode == 'standard':
        setup_standard_split(dataset, args)
    elif split_mode == 'random':
        setup_random_split(dataset, args)
    elif split_mode.startswith('cv-'):
        cv_type, k = split_mode.split('-')[1:]
        setup_cv_split(dataset, cv_type, int(k), args)
    else:
        raise ValueError(f"Unknown split mode: {split_mode}")


def setup_standard_split(dataset, *args, **kwargs):
    ...
    """
    split_index = args[0].split_index
    task_level = args[0].data_task
...
def print_class_distribution(dataset):
    for name, mask in zip(
        ['Train', 'Val', 'Test'],
        [dataset.data.train_mask, dataset.data.val_mask, dataset.data.test_mask]
    ):
        print(f"{name} set class distribution:")
        for cls in torch.unique(dataset.data.y):
            count = (dataset.data.y[mask] == cls).sum().item()
            print(f"  Class {cls}: {count} samples")
            
def setup_random_split1(dataset, args):
...
def setup_random_split(dataset, args):
    """Generate random splits.
    ...
    Raises:
        ValueError: If the number split ratios is not equal to 3, or the ratios
            do not sum up to 1.
    """
    # split_ratios = [0.2, 0.6, 0.2]
    # split_ratios = [0.4, 0.4, 0.2]
    split_ratios = [0.6, 0.2, 0.2]
    # split_ratios = [0.025,0.025,0.95]
    data = dataset.data
    ...
    if len(split_ratios) != 3:
        ...
        raise ValueError(
            f"The train/val/test split ratios must sum up to 1, input ratios "
            f"sum up to {sum(split_ratios):.2f} instead: {repr(split_ratios)}")
    percls_trn = int(round(split_ratios[0] * len(data.y) / dataset.num_classes))
    val_lb = int(round(split_ratios[1] * len(data.y)))
    train_index, val_index, test_index = random_planetoid_splits(
        data, dataset.num_classes, percls_trn, val_lb, args.seed)
